...
describe('useDefaultHomePagePath', () => {
  beforeEach(() => {
    localStorage.removeItem('lastVisitedObjectMetadataItemIdState');
  ...
  it('should return the last-visited object path when one is set', async () => {
    const opportunity = getMockObjectMetadataItemOrThrow('opportunity');

    jotaiStore.set(lastVisitedObjectMetadataItemIdState.atom, opportunity.id);
